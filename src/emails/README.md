...
10. La route interne `/api/guesty/reservation/verify` relit la réservation via Guesty Open API jusqu’à voir le paiement payé. Elle n’appelle pas le endpoint BEAPI `/verify-payment`, réservé au flow GuestyPay SDK avec `threeDSResult`.
11. Si Guesty retourne `FAILED`, la route annule la réservation non payée via OpenAPI avec une raison valide pour libérer les dates
